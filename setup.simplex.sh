...
source "./src/tools.simplex.sh" || exit 1

# showhelp is auto set by tools
tool_simplex_show_help
...
argc=$#
argv=("$@")
tool_simplex_process_arguments ${argc} ${argv}
...
cc_script_make_path="./src/setup.simplex.make.sh"
tool_realpath cc_script_make_path "make script path"
# TODO make simplex is not implemented yet
...
tool_variable_arch_load cc_download_url "simplex pkg download url"
tool_variable_check_load_default cc_download_url "" "simplex pkg download url"
tool_variable_check_load_default cc_download_extracted_bin_files_dir "" "simplex download extracted bin files dir"
...
if [ "${cc_action1}" = "download" ]; then
    ...
    dst=${cc_bin_path}
    tool_cp_dir_recursive src dst "downloaded extracted simplex package"

# copy bin files build from source
elif [ "${cc_action1}" = "build" ]; then
    src=${cc_git_build_bin_file_path}
    dst=${cc_bin_path}
    tool_cp_dir_recursive src dst "git source build simplex"
