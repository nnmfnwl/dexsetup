...
function tool_simplex_firejail_show_help() {
    if [ "${showhelp}" == "yes" ]; then
        echo ""
        echo "*** ABOUT ***"
        echo ""
        echo "Simplex(ultimate privacy messenger) firejail sanboxing profile setup helper script for Debian"
        echo "To have multiple simplex profiles even run them at same time!"
        echo ""
        echo "*** USAGE ***"
        echo ""
        ...
        echo ""
        echo "<path/to/simplex/config/script.sh>"
        ...
        echo " >> ability to have and even run multiple Simplex instances(profiles)"
        ...
        echo ""${0}" ./src/cfg.simplex.sh ./src/cfg.simplex.sh ~/dexsetup/simplex/simplex.v6.4.7"
        echo ""${0}" ./src/cfg.simplex.sh custom_profile_name"
        echo ""
        exit 0
    fi
}

function tool_simplex_firejail_process_arguments() {  #argc  #argv
    argc=${1}
    argv=${2}
    ...
    for (( j=0; j<argc; j++ )); do
        if [ "${argv[j]}" = "nofirejail" ]; then
            ...
            if [ "${cc_script_cfg_path}" = "" ]; then
                ...
                tool_info " " cc_script_cfg_path "custom simplex config path set"
