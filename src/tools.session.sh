...
function tool_session_show_help() {
    if [ "${showhelp}" == "yes" ]; then
        echo ""
        echo "*** ABOUT ***"
        echo ""
        echo "Session(ultimate privacy messenger) setup helper script for Debian"
        echo "To know more about Session, visit getsession.org"
        echo ""
        echo "*** USAGE ***"
        echo ""
        ...
        echo ""
        echo "<path/to/custom/session/config/script.sh>"
        ...
        echo ""${0}" ./src/cfg.session.sh download install ./src/cfg.session.sh ~/dexsetup/session/latest"
        exit 0
    fi
}


function tool_session_process_arguments() {  #argc  #argv
    argc=${1}
    argv=${2}
    ...
    for (( j=0; j<argc; j++ )); do
        if [ "${argv[j]}" = "nofirejail" ]; then
            ...
            if [ "${cc_script_cfg_path}" = "" ]; then
                ...
                tool_info " " cc_script_cfg_path "custom session config path arg"
