...
```
  * successful result of every above command are generated firejail profiles and run scripts:
    ...
    * for example `~/dexsetup/blocknet/firejail.blocknet.wallet_block.d.bin.sh`
  * **it is not recommended to execute generated scripts directly, rather using them with screen setup of this tutorial**
...
```
./setup.cc.wallet.profile.sh ./src/cfg.cc.pivx.sh ~/.pivx/ wallet_pivx_blockdx
...
```
./setup.cc.wallet.profile.sh ... help | less
```
