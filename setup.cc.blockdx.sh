...
source "./src/tools.cc.blockdx.sh" || exit 1

# showhelp is auto set by tools
tool_blockdx_show_help
...
argc=$#
argv=("$@")
tool_blockdx_process_arguments ${argc} ${argv}
...
cc_script_make_path="./src/setup.blockdx.make.sh"
...
tool_variable_check_load_default cc_download_extracted_bin_files_dir "" "blockdx download extracted bin files dir"
...
if [ "${cc_action1}" = "download" ]; then
    ...
    if [ "${cc_action2}" = "install" ]; then
        tool_dir_if_exist_exit cc_git_src_path "checking if src not already installed" "please try update|purge argument"
        tool_dir_if_exist_exit cc_bin_path "checking if bin not already installed" "please try update|purge argument"
        
        tool_make_and_check_dir_path cc_git_src_path "git source directory"
        tool_make_and_check_dir_path cc_bin_path "bin directory"
        
    elif [ "${cc_action2}" = "update" ]; then
        # git src could be actually up to date so do not need to remove pkg dir
        tool_make_and_check_dir_path cc_git_src_path "git source directory"
...
if [ "${cc_action1}" = "download" ]; then
    ...
    dst=${cc_bin_path}
    tool_cp_dir_recursive src dst "downloaded extracted blockdx package"

# copy bin files build from source
elif [ "${cc_action1}" = "build" ]; then
    src=${cc_git_build_bin_file_path}
    dst=${cc_bin_path}
    tool_cp_dir_recursive src dst "git source build blockdx"
...
if [ -f "${clamscan_path}" ]; then
    ...
    ${clamscan_path} --no-summary ${cc_install_dir_path}"/"*
    (test $? != 0) && echo "ERROR >> AV SCAN >> ${clamscan_path} ${cc_install_dir_path}/* >> FAILED" && rm -rf ${cc_bin_path}"/"* && exit 1
    echo "INFO >> AV SCAN >> ${cc_install_dir_path}/* >> success"
fi
