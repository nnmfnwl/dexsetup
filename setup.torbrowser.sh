...
source "./src/tools.torbrowser.sh" || exit 1

# showhelp is auto set by tools
tool_torbrowser_show_help
...
argc=$#
argv=("$@")
tool_torbrowser_process_arguments ${argc} ${argv}
...
cc_script_make_path="./src/setup.torbrowser.make.sh"
...
# where extracted bin files are path
...
# where build bin files are path
...
if [ "${cc_action1}" = "download" ]; then
    ...
    dst=${cc_bin_path}
    tool_cp_dir_recursive src dst "downloaded extracted torbrowser package"

# copy bin files build from source
elif [ "${cc_action1}" = "build" ]; then
    src=${cc_git_build_bin_file_path}
    dst=${cc_bin_path}
    tool_cp_dir_recursive src dst "git source build torbrowser"
...
echo "*** MAIN TOR BROWSER SETUP SUCCESS ***"
